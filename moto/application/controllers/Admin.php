...
class Admin extends CI_Controller
{
        ...
        if ($para1 == 'delete') {
            ...
            foreach ($delivery_status as $row) {
                if (isset($row['admin'])) {
                    ...
                    $new_delivery_status[] = array('vendor' => '', 'status' => $this->input->post('delivery_status'), 'comment' => $this->input->post('comment'), 'delivery_time' => time());
                }
            }
            $payment_status = json_decode($this->db->get_where('sale', array(
                'sale_id' => $para2
            ...
            foreach ($payment_status as $row) {
                if (isset($row['admin'])) {
                    ...
                    $new_payment_status[] = array('vendor' => '', 'status' => $this->input->post('payment_status'));
                }
            }
 ...
 /////////////////////////   /* Membership Management */
    function membership($para1 = '', $para2 = '',$para3='')
    {
        ...
        $products   = $this->db->get('membership')->result_array();
        $data       = array();
        foreach ($products as $row) {
            ...
            if ($row['status'] == 'ok') {
                $res['publish'] = '<input id="pub_' . $row['membership_id'] . '" class="sw1"  type="checkbox" data-id="' . $row['membership_id'] . '" checked />';
            } else {
                $res['publish'] = '<input id="pub_' . $row['membership_id'] . '" class="sw1" type="checkbox" data-id="' . $row['membership_id'] . '" />';
            }
   ...
   /* Manage Frontend Captcha Settings Credentials */
    ...
    function business_settings($para1 = "", $para2 = "", $para3 = "")
    {
        if (!$this->crud_model->admin_permission('business_settings')) {
            ...
            $this->db->where('type', "tax_cost_type");
            $this->db->update('business_settings', array(
                'value' => $this->input->post('tax_cost_type')
            ));
            $this->db->where('type', "tax_cost");
            $this->db->update('business_settings', array(
                'value' => $this->input->post('tax_cost')
            ));
