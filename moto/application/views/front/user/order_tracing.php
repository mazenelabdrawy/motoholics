...
<?php
		foreach ($delivery_status as $row) {
            ...
            if(isset($row['admin'])){
            	...
            	$status_updated_on = date('d m, Y h:i:s A',$delivery_time);
            	$from = $this->crud_model->provider_detail('vendor','','with_link');
            	$delivery_status_p = $row['status'];
            	$delivery_status_c = '('.$row['comment'].')';
			}
?>
		<tr>
			<td>
				...
				<b><?php echo translate('delivery_status'); ?> : </b> <?php echo $delivery_status_p; ?> <?php echo $delivery_status_c; ?><br><br>
